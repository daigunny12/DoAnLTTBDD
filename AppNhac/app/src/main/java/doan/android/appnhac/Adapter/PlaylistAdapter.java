...
public class PlaylistAdapter extends ArrayAdapter<Playlist> {
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        if(convertView == null){
            ...
            viewHolder.imgplaylist = convertView.findViewById(R.id.imgeviewplaylist);
            convertView.setTag(viewHolder);
        }else {
             viewHolder = (ViewHolder) convertView.getTag();
        }
        Playlist playlist = getItem(position);
        Picasso.with(getContext()).load(playlist.getPlaylistImage()).into(viewHolder.imgbackground);
        Picasso.with(getContext()).load(playlist.getPlaylistIcon()).into(viewHolder.imgplaylist);
        viewHolder.txttenplaylist.setText(playlist.getPlaylistName());
